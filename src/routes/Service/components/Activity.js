...
const Activity = React.createClass({
  ...
  sunbmitOrder() {
    ...
    var customer = $('.acustomer.input input').val();
    var buffer = this.state.buffer;
    var obj = {};
    ...
    return(
      <div className='active content'>
        ...
        <div className='ui basic segment' style={ style.section }>
          ...
          <div className='ui acustomer input'>
            <input type='text' />
          </div><br/><br/>
...
const BRow = React.createClass({
  componentDidMount() {
    $(`.dropdown.a${this.props.index}`).dropdown({
  ...
  updateData(source) {
    var stationery = $(`.dropdown.a${this.props.index}`).dropdown('get value');
    var amount = $(`.aamount.input.a${this.props.index} input`).val();
    var note = $(`.anote.input.a${this.props.index} input`).val();

    if(isNaN(parseInt(amount)) && source) {
      alert('數量請填寫數字!');
      $(`.aamount.input.a${this.props.index} input`).val('');
      return;
    }
  ...
  render() {
    return(
      <tr>
        <td>
          <select className={`ui activity dropdown a${this.props.index}`}>
          {
        ...
        <td>
          <div className={`ui fluid aamount input a${this.props.index}`} onBlur={()=>{this.updateData(true)}}>
            <input type='text' />
          </div>
        </td>
        <td>
          <div className={`ui fluid anote input a${this.props.index}`} onBlur={()=>{this.updateData(false)}}>
            <input type='text' />
          </div>
        </td>
